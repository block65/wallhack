...
class WallhackProcess:
    ...
    def wait_for_tun(self, ns: str | None = None) -> None:
        ...
        while time.monotonic() < deadline:
            ...
            if netns.link_exists(target_ns, TUN_NAME):
                # Wallhack created it in DOWN state -- bring it up
                netns.set_link_up(target_ns, TUN_NAME)
                return
            time.sleep(TUN_POLL_INTERVAL)
        out = self.output()
        raise TimeoutError(
            f"TUN interface {TUN_NAME} did not appear in {target_ns} "
