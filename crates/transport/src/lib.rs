...
//!
//! This module provides a transport-agnostic interface for multiplexed connections.
//! The [`Transport`] trait abstracts over different transport mechanisms (QUIC, WebSocket+yamux)
//! allowing the server and client to work with any compatible transport.
...
pub trait BiStream: AsyncRead + AsyncWrite + Send + Unpin {
	/// Finishes the write half of this stream, signaling no more data will be sent.
	///
	/// This is similar to [`AsyncWriteExt::shutdown`] but may have transport-specific
	/// semantics (e.g., QUIC's `finish()` sends a FIN frame).
	fn finish(&mut self) -> impl Future<Output = Result<(), TransportError>> + Send;
}

/// A multiplexed transport connection.
///
/// This trait abstracts over different transport mechanisms (QUIC, WebSocket+yamux, etc.)
/// providing a unified interface for opening and accepting streams.
///
/// # Stream Types
///
/// Transports support two types of streams:
/// - **Unidirectional**: One-way streams for sending data (opened via [`open_uni`][Self::open_uni])
///   or receiving data (accepted via [`accept_uni`][Self::accept_uni]).
/// - **Bidirectional**: Two-way streams for request-response patterns
///   (opened via [`open_bi`][Self::open_bi], accepted via [`accept_bi`][Self::accept_bi]).
pub trait Transport: Send + Sync {
	...
	/// # Errors
	///
	/// Returns an error if the connection is closed or the transport cannot open more streams.
